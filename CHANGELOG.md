...
### 安装包重建与运行时修复
- 以当前 Unity 完整构建重组 portable 目录，并重新生成 EXE、ZIP 与 SHA256 发布元数据。
- portable 中的 Ollama 下载脚本改为 UTF-8 BOM，确保 Windows PowerShell 5.1 能正确解析中文脚本。
- 随包 Node 固定为 v22.22.3，安装器构建会显式重组该运行时，不依赖机器 PATH；`version.txt` 与安装器发布版本一致。
- 本机构建、portable 脚本解析与静默文件安装已验证；NSSM 仍在安装时在线下载，下载失败会使桥接服务未注册，干净机服务验收与正式签名仍未完成。

## Unreleased（2026-08-30）

### 像素节日适配
- 新增可切换的新春主题：为 17×24 像素符玄叠加程序化红金帽饰，并同步聊天窗口与聊天气泡配色。
- 将节日视觉改为完整 `ThemeSkin` 皮肤重建，覆盖背景、星云、星点、按钮、输入栏、子面板和气泡，避免旧紫色纹理串入；透明配饰层不修改基础像素帧。
- 新增测试模式命令 `@@sim:holiday:cn_new_year/off/auto/status/list`，支持运行时截图对照。
- 本轮不修改 Live2D 模型、参数、物理或渲染链路。
...
- 外置 Unity 输入字层增加长文本水平视口，光标接近右边界时自动跟随，删除/左移时自动回收。
- 点击 Unity 渲染字层会按实际字体宽度定位隐藏 Win32 输入宿主的 UTF-16 插入点。
- 安装包构建流程新增 EXE/ZIP SHA256 文件；浏览器拦截 EXE 时可下载 ZIP 备用包。
- 增加受信任 PFX + `signtool` 的可选签名流程；未配置证书时明确提示不伪造签名。
...
- 本地模型新增意图分类、紧凑工具规划、白名单校验和统一 ToolEngine 执行链路。
- 支持通过自然语言触发系统信息、文件搜索、打开文件夹、剪贴板、动作和办公工具。
- 增加规划 JSON 重试、无参数工具容错和高置信度任务复核；危险工具继续要求用户确认。
- 增加隔离验收脚本，测试不会写入生产忆境。
...
- 修复外置聊天窗口使用中文输入法时，候选框错误出现在屏幕左上角的问题。
- 将 Unity 输入栏坐标正确转换到隐藏原生输入控件的客户区坐标。
- 输入法组合过程中采用异步重定位，避免 IMM 回调递归导致原生崩溃。
- 已通过 `BuildScript.VerifyCompile` 和完整桌宠构建验证。
...
- 安装包更新至最新桌宠构建，修复中文输入法定位逻辑导致的 Mono 原生崩溃。
- 便携目录内置 Node v22.22.3，并重新打包 OpenClaw 桥接依赖。
- 完成静默安装、文件落地校验和静默卸载验证。
