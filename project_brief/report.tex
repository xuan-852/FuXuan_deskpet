...
\begin{abstract}
本文档为符玄桌面宠物项目的完整技术报告（V2.4）。项目基于 Tuanjie 2022.3.62t7（团结引擎）与 Live2D Cubism SDK 5-r.4，
构建了一个运行于 Windows 桌面的 AI 驱动虚拟角色。系统深度集成 DeepSeek Chat API（含 Function Calling 48+ 工具）、
GLM-4V 多模态视觉模型、Ollama 本地 LLM 实时决策引擎、本地知识库（RAG）以及多层级感知与记忆系统。
核心亮点是**具身智能闭环**——从感知、决策、执行到视觉验证与记忆学习的完整循环。

V2.4 在 V2.3+ 基础上引入了藏书阁本地知识库 RAG 系统，覆盖悬浮球与右侧面板交互体系、MotionAgent 自主动作决策引擎、GLM-4V 多帧拼图视觉验证、
Newtonsoft.Json 序列化重构、GpuLoadMonitor 游戏检测等高级特性，涵盖物理模拟、Live2D 参数化动画、
DWM 透明窗口、AI 对话、天气感知、便签提醒、本地知识库等核心功能。
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{AI 对话}：DeepSeek Chat API + Function Calling（48+ 法术工具）
...
48+ 个注册工具通过 JSON Schema 暴露给 DeepSeek，覆盖网页搜索、截图分析、
...
\begin{longtable}{lll}
\caption{工具列表（48+）\label{tab:tools}}\\
\toprule
...
\textbf{V2.4} & \textbf{2026-07-14} & \textbf{藏书阁 — 本地知识库 RAG 系统（KnowledgeBaseManager）；Ollama nomic-embed-text 语义检索；自动对话关联检索注入 SystemPrompt；knowledge\_search / knowledge\_index 工具（48+）} \\
\bottomrule
\end{tabular}
\end{table}
...
验证日志 & \texttt{D:\DesktopPetData\validation\_log.json} \\
