...
> **像素节日适配记录（2026-08-30）**: 已完成像素符玄与聊天窗口/气泡的新春红金主题。主题由完整 `ThemeSkin` 管理，RightPanel/子面板/ChatBubble 全量重建皮肤纹理，像素帽以独立透明层合成，支持测试命令切换和截图对照；Live2D 明确保持不变。Quick、完整构建、隔离冒烟和最终播放器截图闭环通过。
