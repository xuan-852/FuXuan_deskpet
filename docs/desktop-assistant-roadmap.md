...
> **性能实施记录（2026-08-29）**: Live2D 参数写入已改为模型加载后缓存 `CubismParameter` 引用，`ForceUpdateNow()` 已统一进入可观测统计入口；本轮保留原有物理顺序和调用条件。另将 RightPanel 星空运动更新移出 `OnGUI`，保持绘制事件无状态副作用。Quick/完整构建/隔离运行时冒烟通过，性能收益仍需可见播放器 Profiling。
