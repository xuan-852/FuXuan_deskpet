...
9 种 JSON 配置驱动（id 1--9：歪头 / 微笑 / 挑眉 / 星辉* / 伸懒腰 / 委屈 / 法阵* / 害羞 / 困惑；*为硬编码特效）。7 种普通动作使用语义参数键，由 `Live2DParameterMapper` 写入；调度器用 Newtonsoft.Json 读取字典目标并复用逐帧目标缓冲。天气/时段动态调整权重：夜晚微笑×0.3 / 雨哭×1.8 / 雪微笑×1.5；冷却系统防复读。

## 三、开发历史迭代

- 2026-09-17：认证技能库新增独立的 `LlmExposed` 白名单。运行时认证不再自动等于 AI 可调用；聊天提示和 `request_body_skill` 均只展示并接受显式公开的语义技能，未公开内部候选保持不可见。
...
| 2026-08-29 | 2026-08-29 | 普通空闲动作迁移到语义参数键；修复 Unity `JsonUtility` 不支持字典目标的问题；复用目标/冷却缓冲并修复 `star_spin.json` 重复尾段；Quick/完整构建、隔离冒烟和 `@@idle:1` 截图闭环通过 |
...
5. **行走循环研究未落地**：`walk_cycle_research.md`（Animator Island / 《动画师生存手册》理论）是纯研究笔记，Live2D 模型无腿部参数，不能把它当作 AI 生成的腿部走路能力；但 `DesktopPet` 已有独立的地面状态机，会做真实横向位移并由渲染器提供身体步态。当前采用用户确认的原始节奏：24px/s、单次移动 2–4 秒、静止 6–15 秒；设置页保存的仅是任务权重，不覆盖这组默认节奏。
