...
| 2026-08-21 | **外置 UI 升帧**：打开外置聊天窗口时 `PerformanceMonitor` 临时切换为 High/Normal/Low = 60/45/30 FPS，关闭后当时恢复后台 30/24/15 FPS；2026-08-29 Live2D 局部 RT 优化后，后台目标调整为 60/45/30 FPS；高负载仍允许自动降档，RenderTexture 推送频率跟随当前档位。 |
...
- `@@sim:screenshot:<label>` 优先请求 Unity 窗口重绘并从自身 RenderTexture 保存面板 PNG；透明层回读不可用时才使用窗口/面板区域兜底，禁止把桌面截图当正式视觉证据。
- 端午主题保留原始像素符玄，不再合成头像叶饰；`DrawMascotAvatar` 在头像外绘制竹叶、金线和水波式头像框。`HolidayFireworksField` 在右侧聊天内容区绘制龙舟、龙头、旗帜、船桨、新荷、两侧艾草、酒盏和低对比度端午诗句；龙舟按实际运动方向成组换向，船头不会倒着走。
- 新春主题将默认的 `StarField` 星空/流星层替换为独立 `HolidayFireworksField`：低密度红金像素烟花依次升空、快速爆裂并渐隐，周期约 3～4 秒，爆点上下分层分布，单朵爆裂半径约为面板短边的 12%～20%；火星带轻微角度扰动，使用初速、重力项和分段短尾迹形成抛物线下坠，避免规则齿轮状放射。其余节日使用同一绘制入口切换到月柳灯市、水波、星桥和月兔动态层；每个主题都使用独立节日色槽，切回默认主题后恢复星空，避免两套背景叠加。
- 元宵主题以欧阳修《生查子·元夕》为画面骨架：右侧内容区绘制七段像素月轮、垂柳、花市横梁和上下三层共 8 盏灯笼；词句按“去年元夜时 / 花市灯如昼 / 月上柳梢头 / 人约黄昏后”从右向左竖排，列内自上而下，使用行楷/行书字体回退、18～42px 响应式字号、4～7 秒呼吸亮度和 1～2px 中轴线轻微偏移，且避开标题、导航和输入区。
...
- 最新春节单主题闭环测试和 static/motion/recovery 人工检查通过；本轮完整构建 CPU 峰值 24%，使用 16/32 逻辑核负载保护。

### 中秋最新复核补充（2026-09-01）

- 中秋主题保留月亮主视觉，并补强右侧聊天区的云朵、玉兔和桂枝桂花前景；月光呼吸、云朵慢移和玉兔轻跳构成环境与事件动效。
- 最新中秋单主题闭环测试和 static/motion/recovery 人工检查通过；本轮完整构建 CPU 峰值 17%，使用 16/32 逻辑核负载保护。

### 七夕最新复核补充（2026-09-01）
- 2026-09-04 复测将七夕收敛为“静态构图＋呼吸灯”：高对比拱形鹊桥、两端双星、纤云和两只像素鹊影固定在聊天内容区，移除流星位移和星桥上下移动；诗词只做整列透明度呼吸，字符位置保持稳定，避免小界面卡顿。
- 最新代码提交为 `df8b80e`。`holiday_eval_drive.cjs --theme=qixi --verbose` 重新通过，生成 `qixi_static`、`qixi_small`、`qixi_motion`、`default_recovery` 四张 Unity 截图；`list/status/off`、`@@test:quit` 日志齐全，截图目录为 `%TEMP%/fuxuan_qixi_final_eval_20260904/test_screenshots/`。
- 后续星光微调提交为 `225accb`：扩大普通星点和重点星的亮度呼吸范围，并为重点星增加金色十字光晕；不改变鹊桥、诗词和静态字位，也不增加位置移动。

- 七夕星点限制在右侧聊天区，采用少量高亮星和十字闪烁；鹊桥由连续桥段连接两颗端点星，避免仅显示零散横线。
- 最新七夕单主题闭环测试和 static/motion/recovery 人工检查通过；本轮完整构建 CPU 峰值 27%，使用 16/32 逻辑核负载保护。

### 元宵最新复核补充（2026-09-01）

- 元宵灯笼限制在右侧聊天主视图，采用上下三层布局、明确挂线和较高亮度；月轮、柳枝和花市横梁均避开标题与导航安全区，避免左侧会话列表或交互控件出现节日残影。
- 元宵词句使用“去年元夜时 / 花市灯如昼 / 月上柳梢头 / 人约黄昏后”四列竖式排版，右向左阅读，列内自上而下，字体使用行楷/行书回退，亮度呼吸且字符只做中轴线附近的小幅偏移。
- 最新元宵单主题闭环测试和 static/motion/recovery 人工检查通过；本轮 Quick 构建 CPU 峰值 24%，完整构建 CPU 峰值 29%，均使用 16/32 逻辑核负载保护。
...
- 设置页提供“桌宠回到左侧”；它将本体带回默认 `startX` 停靠点并持久化。拖拽完成后只保存横向相对位置，重启时仍按地面物理落地，避免保存不会保持的空中高度。
- 隔离测试可通过 `@@sim:drag:offset:dx,dy,steps` 后写入 `@@view:pet-home` 验证位置文件和恢复行为。
> **2026-09-14 稳定发布基线**：聊天列表、聊天页与子面板依据 Windows 主屏工作区计算尺寸和位置，并额外保留边缘安全区。窗口在拖动、切换视图或工作区不足时完整夹紧，避免任务栏遮挡关闭、返回、帮助和输入区。
