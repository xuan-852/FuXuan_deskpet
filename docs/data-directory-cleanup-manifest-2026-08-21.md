# 用户数据目录整理清单（2026-08-21）

## 目的

符玄运行时的唯一用户数据根目录由 `DataPathConfig.DataRoot` 决定：新安装默认是 `%LOCALAPPDATA%\FuXuan\DesktopPetData`，也可通过 `FU_XUAN_DATA` 指定；本清单中的 `D:\DesktopPetData` 是 2026-08-21 整理时使用的显式数据根目录示例。整理只收拢可识别的测评、诊断和旧备份产物，不删除内容，不移动当前运行所需的数据文件。
