...
本轮节日适配（2026-08-30）：完成完整 `ThemeSkin` 皮肤重构，像素符玄、聊天 UI、气泡和子面板统一切换新春红金配色；像素帽使用独立透明层，支持 `@@sim:holiday:cn_new_year/off/auto/status/list`，并用隔离播放器完成开启/关闭截图闭环。Quick、完整构建、隔离冒烟均通过；Live2D 未修改。
