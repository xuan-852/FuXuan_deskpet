...
本轮前端体验优化（2026-09-11）：保留现有真实横向走路和 Live2D 身体步态，只提高新用户默认的可见性：速度 24→36px/s，连续移动 2–4→2.5–4.5 秒，停留 6–15→3.5–7.5 秒；单段约 90–162px。Quick 与隔离 EditMode（161 passed、1 ignored、0 failed）通过；透明置顶窗不会出现在常规桌面截图中，最终观感仍以可见桌宠人工观察为准。
