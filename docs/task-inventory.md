...
本轮前端体验优化（2026-09-11）：保留现有真实横向走路和 Live2D 身体步态，提高新用户默认的可见性：速度 24→36px/s，连续移动 2–4→2.5–4.5 秒，停留 6–15→3.5–7.5 秒；单段约 90–162px。身体步态同步增强：重心 4→8px、横摆 2→4、转体摆幅 3→5、手臂/肩膀振幅提高，并新增仅测试模式可用的 `@@sim:walk:left|right|stop` 强制走路验收入口。Quick、隔离 EditMode（161 passed、1 ignored、0 failed）与隔离运行时冒烟通过；透明置顶窗不会出现在常规桌面截图中，最终观感仍以可见桌宠人工观察为准。
