...
> 文件版本: N40+ · 最后更新: 2026-09-04（五个传统节日完成逐主题截图、命令日志和视觉预审；文档同步提交 `96f199d`，T3/T5 待真实 GUI 签字，T6 暂不关闭）
>
...
| ✅ | EditMode 新鲜测试结果 | **已修复（2026-08-31）**：`build.ps1 -RunTests` 曾因同时传 `-quit` 测试运行器提前退出、不写结果；已去掉 `-quit` 并加结果文件门禁（跑前删旧、跑后校验 `failed=0`）。实测 160 用例 **0 失败**（1 环境忽略） |
| ⚠️ | Tuanjie/Unity 构建复核 | 受限环境可能外层卡住；必须在本机权限下以新日志和 `[OK] Build succeeded!` 判定 |
| ⚠️ | 停走头部稳定性与问候语时段人工验收 | 代码、Quick、完整构建和隔离冒烟已通过；仍需可见播放器连续停走和调试小时抽查 |

本轮构建/测试修复（2026-08-31）：① `build.ps1` 内置构建负载保护（默认增量保留 `Library\Bee` + `-MaxCores` 限制 16 核 + BelowNormal + 2s 子进程监视，实测完整构建 CPU 均值 15~19%/峰值 21~26%）；② `-CleanBeeCache` 显式清缓存开关；③ 修复 `-RunTests` 的 `-quit` bug 并加结果文件门禁；④ 由「`-RunTests` 不再产出结果」暴露出的 4 个隐藏失败已修复——`MemoryGovernance` 合并测试输入过闸门、`LocalToolRouter` 危险工具排除出本地自动白名单（`file_delete` 等需审批）、`LocalRoleplayPromptBuilder` 提示词断言、`DataPathConfig` 环境依赖断言；⑤ 新增 `BuildScriptBeeCacheTests`。全量 EditMode 套件已全绿。
...
本轮第六步（2026-08-30）：修复走路停止时普通额外 `ForceUpdateNow()` 重复推进物理导致的头部抖动风险；修复 `AutoChat` 未跟随调试小时、`IdleChatGenerator` 旧异步问候跨时段入队的问题；构建脚本增加 ILPP PID 身份识别，避免无关 `conhost` 复用 PID 永久阻塞。Quick、完整构建和隔离运行时冒烟均通过；停走观感与问候自然语言仍待人工确认。

本轮节日适配（2026-08-30～09-01）：完成完整 `ThemeSkin` 皮肤重构，像素符玄、聊天 UI、气泡和子面板支持 5 个中国传统节日主题；每个主题使用独立透明配饰层与动态背景，分别覆盖烟花、月柳灯市、水波、星桥和月兔；元宵按欧阳修《生查子·元夕》完成诗词驱动画面、竖式排版和大界面响应式字号；支持 `@@sim:holiday:<theme>` 及 `off/auto/status/list`，并用隔离播放器完成逐主题截图闭环。Quick、完整构建、隔离冒烟均通过；Live2D 未修改。

本轮前端体验复核（2026-09-11）：可见播放器中确认原始走路节奏与身体幅度更自然，因此恢复 24px/s、移动 2–4 秒、停留 6–15 秒、5rad/s 步频与 4px 重心起伏。仅新增测试模式的 `@@sim:walk:left|right|stop` 强制走路验收入口，不改变正式用户的走路观感；恢复后须重新完成 Quick、隔离 EditMode 与隔离运行时冒烟。

本轮聊天头像统一（2026-09-11）：聊天标题、气泡、输入框、会话列表和侧栏改为保持 17×24 像素立绘比例，默认链路使用 Point 采样并增加统一像素边框，移除高清头像 Bilinear 缩放与正方形裁切造成的风格割裂。Quick、隔离 EditMode、隔离完整运行时冒烟和实际聊天截图均已通过；正式 `D:\Fuxuan` 未覆盖。
...
| 目标 ID | 目标 | 完成定义 |
|---------|------|----------|
| G1 | 完成 5 个中国传统节日的诗词驱动皮肤 | 新春、元宵、端午、七夕、中秋分别有已确认诗词和独立验收记录 |
| G2 | 保证视觉完整和响应式可用 | 主视觉、三层空间、配饰、动效、小/大界面均通过人工检查 |
| G3 | 建立可追溯验收闭环 | 每个主题具备构建、隔离冒烟、四图截图、日志、评分和问题记录 |
| G4 | 保持工程安全边界 | 不修改 Live2D、默认主题和生产数据，不执行危险工具或外部调用 |
| G5 | 控制主题范围 | 不恢复元旦、万圣节、圣诞节等当前正式范围之外的主题 |

### 单节日任务卡

任务阶段统一为：T0 诗词确认 → T1 画面设计 → T2 代码实现 → T3 小/大界面适配 → T4 自动验证 → T5 人工审核 → T6 文档归档与提交。代码存在只计入 T2，不自动计入 T4～T6。

| 节日 | 主题 ID | T0 | T1 | T2 | T3 | T4 | T5 | T6 | 总状态 |
|------|---------|----|----|----|----|----|----|----|--------|
| 新春 | `cn_new_year` | ✅ | ✅ | ✅ | ⏳ | ✅ | ⏳ | ⏳ | ⏳ 自动证据齐全、预评分 91/100；待真实 GUI 双击/拖拽签字 |
| 元宵 | `lantern_festival` | ✅ | ✅ | ✅ | ⏳ | ✅ | ⏳ | ⏳ | ⏳ 自动证据齐全、预评分 92/100；待真实 GUI 双击/拖拽签字 |
| 端午 | `dragon_boat` | ✅ | ✅ | ✅ | ⏳ | ✅ | ⏳ | ⏳ | ⏳ 自动证据齐全、预评分 91/100；待真实 GUI 双击/拖拽签字 |
| 七夕 | `qixi` | ✅ | ✅ | ✅ | ⏳ | ✅ | ⏳ | ⏳ | ⏳ 自动证据齐全、预评分 91/100；待真实 GUI 双击/拖拽签字 |
| 中秋 | `mid_autumn` | ✅ | ✅ | ✅ | ⏳ | ✅ | ⏳ | ⏳ | ⏳ 自动证据齐全、预评分 91/100；待真实 GUI 双击/拖拽签字 |
...
### 2026-09-11 首启与本地版本中心

| 状态 | 任务 | 验证依据 |
|------|------|---------|
| ✅ | 新用户轻量首启提示、欢迎三步、可选称呼与明确自启；旧用户静默分流；聊天/托盘帮助入口 | `build.ps1 -Quick`、新增 EditMode 状态/版本回退测试、隔离 `runtime_smoke.cjs --ui-experience-only --verbose` 通过 |
| ✅ | 设置页“关于与数据”：本地版本/发布说明、运行与数据目录、可写性、自启、打开/复制路径；更新保留说明 | 独立输出完整构建、欢迎/版本中心截图、零 NRE、生产数据零污染 |
| ✅ | portable 构建随包生成 `version.txt` 与 `release-notes.txt` | PowerShell 5.1 语法解析通过；正式发布仍需按现有安装器流程进行覆盖安装验收 |
