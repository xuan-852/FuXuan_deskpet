...
本轮节日适配（2026-08-30～31）：完成完整 `ThemeSkin` 皮肤重构，像素符玄、聊天 UI、气泡和子面板支持 8 个节日主题；每个主题使用独立透明配饰层与动态背景，分别覆盖烟花、灯笼、水波、星桥、月兔、南瓜/蝙蝠、飘雪/彩灯和元旦彩带；支持 `@@sim:holiday:<theme>` 及 `off/auto/status/list`，并用隔离播放器完成 8 个主题截图闭环。Quick、完整构建、隔离冒烟均通过；Live2D 未修改。
