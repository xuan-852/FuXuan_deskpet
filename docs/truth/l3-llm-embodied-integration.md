...
加上此前 `external_Hiyori_Hiyori_m02`（85/92）与 `screen_side_arm_raise`（85/92 单臂上抬），**认证技能库共 6 项**，登记于 `CertifiedMotionLibrary`（数据层，包指纹齐全）。
...
- **LLM 工具** `request_body_skill`（ToolEngine）：描述自动枚举认证技能与语义边界；只接受白名单内 skill_id，未认证请求即终态拒绝；无原始参数入口。已加入 `LocalToolRouter` operation 意图白名单（本地与云端 Function Calling 同源）。
- **测试链路** `@@sim:certified-motion:<skillId>`（仅 `.test_mode`）供确定性隔离驱动。

## 可复核证据（2026-09-17）

- `build.ps1 -RunTests`：EditMode **213 用例、212 passed、failed=0**、1 ignored；新增 `CertifiedMotionLibraryTests`（条目有效性、全部注册准入、工具注册与白名单）与 `EmbodiedMotionCurveTests`。
...
- LLM 只能选择认证技能、按语义边界描述请求；拒绝是终态；结果以开始/拒绝状态返回，完成事实由运行时收束日志承载（C-L3-06 不回传参数轨迹）。
