...
## 2026-09-17 躯干与手臂机械复审（物理写入对照）

- 独立 `Live2DProbe.exe` 在隔离 `.test_mode` 根目录内完成固定骨架组合三轮 `baseline / member-min / member-max / combined corners / reset` 采样；冻结与保留 `CubismPhysicsController` 两种模式均正常退出，全部组合 `resetStable=true`、最大复位像素差为 `0`。
- 正负控制同批三轮复验：`ParamAngleZ` 在冻结/物理模式的成员峰值分别为 `7.176` / `7.767`，`Param911` 两个方向均为 `0`，故本批能够区分有效写入和不可见写入；控制结论仅证明测量链路有效，不授予任何参数语义或认证。
- 物理模式的躯干候选：`ParamBodyAngleX/Y/Z` 成员峰值分别为 `5.376`、`13.831`、`27.043`；人工帧审查确认 `ParamBodyAngleZ` 双向极值为可复位的全身侧倾，未见明显穿模或残留。它们保留为后续受限幅的躯干语义候选，仍为 `not-certified`、`mapWriteAllowed=false`。
- 冻结模式的手臂候选：`Param31`/`Param34` 峰值为 `1.417`/`1.328`，`Param32`/`Param36` 为较弱变化，`Param33`/`Param37` 接近零；但在物理模式下 `Param31/32/33/34/36/37` 全部为 `0`。因此当前手臂结果只说明存在“被物理写入者覆盖”的离线候选，不能进入运行时躯干/手臂控制层，也不能用于动作认证。
- 探针组合报告已增加成员级最小/最大像素差字段，以免把同组头部或躯干的变化错误归因给手臂成员。
