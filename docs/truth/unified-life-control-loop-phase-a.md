...
- 表情入口增加 `TryPlayExpression` 成功语义：未知表情不会在验证失败后遗留输入租约；Renderer 禁用和应用退出共用幂等外部输入清理，表情租约与延迟回调会被收束。

## 验证证据

- `build.ps1 -Quick`：2026-09-19 通过，`[OK] Build succeeded!`。
- `build.ps1 -RunTests`：本轮新生成结果为 `total=242`、`passed=241`、`failed=0`、`ignored=1`；含新增认证拒绝终态、终态幂等、抢占、PoseState 异常恢复及 Phase B 观测基础测试。
