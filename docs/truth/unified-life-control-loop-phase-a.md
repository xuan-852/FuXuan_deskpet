...
- `BodyWriterInventory` 登记外部请求租约 writer、内部叠加层、桌面状态租约及 Renderer 参数提交层，并分别记录 `Role` 与 `ControlLevel`：`certified-motion` 为 `CertifiedCoordinator/ExternalLeaseWriter`；表达、旧动作、生成动作、空闲、步行和拖拽响应为 `InputLeaseOnly/ExternalLeaseWriter`；`desktop-physics` 为 `InputLeaseOnly/DesktopStateLease`（租约用于登记/协调，不阻止 PhysicsRoot 继续步进）；`mouse-gaze` 为 `InternalOnly/LeaseGatedOverlay`，不取得独立租约；`renderer-parameter-commit` 为 `InternalOnly/InternalParameterLayer`，表示 Renderer 内部多类参数写入最终经 `ParameterCommitBridge` 提交，并非单一动作。该清册是迁移与审计依据，**不是**参数白名单、能力注册表或“已经统一控制”的声明。
