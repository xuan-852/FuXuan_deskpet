...
> **状态**：认证、表情、旧动作、生成动作、空闲、步行、桌面物理和拖拽响应均已绑定到注册写入者；仍采用全局单租约，生命周期与全路径验收未完成。
...
- `drag-response` 已登记为 `InputLeaseOnly`，由 `DragHandler` 持有其租约并在正常释放、失焦、禁用、销毁和退出路径幂等清理；`Live2DRenderer.SafeRecoverExternalActions` 也会主动调用拖拽恢复入口。当前仍不等于 Renderer 重建、真实 OS 鼠标或认证动作双向 handoff 已完成。
...
- `drag-response` 的代码层租约和 teardown 清理已补齐；资源级并行或抢占仍未引入。
