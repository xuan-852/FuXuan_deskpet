...
> **动态重绘修复（2026-09-04，代码提交 `b531d03`）**：节日动态时间统一由 `HolidayFireworksField.UpdateMotion()` 使用 `Time.unscaledDeltaTime` 推进，绘制阶段不再读取 `Time.time` 推进状态；`RightPanel.Update()` 按当前性能档位以 30～60 FPS 请求透明窗口重绘。诗词呼吸亮度扩大到可见范围，背景位移和字符微动保持连续；这项修复不改变主题接口、生产数据边界或 Live2D 链路。

> **五主题视觉优化复测（2026-09-04）**：在上述动态时钟基础上，`HolidayFireworksField` 增加了按面板宽度的紧凑布局安全区（宽度 `< 700` 时避开标题/会话列表/输入栏），五主题诗词列改用面板宽度判断，避免高而窄的窗口误用大排版。新春补充低对比门窗背板并延长烟花可见相位；元宵增加灯笼事件点亮、底部倒影并收紧灯笼与诗词的横向关系；端午放大并减速龙舟、减少重复水纹；七夕提高诗词呼吸可读性；中秋按大小窗口调整月轮并放大大窗口玉兔、增强桂枝层次。最终隔离评测生成五主题各 `static/small/motion/default_recovery` 共 16 张有效 Unity 截图，未发现异常；本记录不替代真实 GUI 双击展开和拖拽/收回签字。
...
> **七夕稳定动效补充（2026-09-04，代码提交 `df8b80e`）**：七夕主视觉固定为高对比像素鹊桥、牵牛/织女双星、纤云和桥上鹊影；不再使用流星位移动画或星桥上下位移动画。七夕动态仅保留星点与诗词整列透明度呼吸，诗词字符位置保持静止，仅保留中轴线附近的固定小幅错位，以避免小界面出现卡顿和文字跳动。主视觉由 `qixi` 独立绘制，不改变主题接口、Live2D 或默认恢复链路。

> **七夕星光复测补充（2026-09-04，代码提交 `225accb`）**：普通星点与重点星的呼吸亮度范围已扩大，重点星增加金色十字光晕；星点位置、鹊桥、诗词和其他构图保持不变，仍只使用透明度变化，不引入位移动画。
