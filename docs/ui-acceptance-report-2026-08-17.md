...
## 十五、遗留面板与独立窗口入口修复（2026-08-17）

针对本轮人工验收反馈完成修复：

- 停止自动创建遗留 `BallPanel`；如果场景中已有该组件，启动时立即关闭并禁用，避免左下角偶发出现“符玄·太卜司”旧系统面板。
- `~`、F2、反斜杠热键统一直接唤出独立聊天窗口；再次触发时请求完整关闭，不再只切换旧的内嵌面板。
- 独立窗口渲染时隐藏顶部“独立”切换按钮，内嵌面板仍保留入口。
- 保持 Unity Live2D 主窗口为置顶层，独立聊天窗口为普通 Windows 窗口；不再用透明层挖洞裁剪 Live2D，修复模型走到独立面板区域被截断的问题。
- 独立窗口点击 X 只触发一次关闭回调，统一隐藏原生窗口、退出外置模式并关闭 Unity 面板，修复关闭不完全和旧面板残留。

自动验证：`build.ps1 -Quick` 通过；EditMode 78/78 通过；隔离 `runtime_smoke.cjs --verbose` 通过；完整构建通过。最新构建待人工复核热键、Live2D 穿越独立窗口区域及 X 关闭后的桌面状态。

## 十六、下方 X 关闭与外置标题栏合并修复（2026-08-17）

本轮截图复核确认：用户所说的 X 是聊天面板标题行下方的 X，不是旧外置标题栏里的关闭控件。此前该按钮只调用 Unity 面板淡出，外置窗口仍保留最后一帧紫色 RT，造成关闭后残留。

本轮修复：

- 外置模式下聊天/会话标题行的 X 直接请求关闭外置窗口，不再调用 Unity 内嵌面板 `Close()`。
- 外置窗口关闭回调立即清空 Unity 面板可见状态，再解除外置模式，两个窗口生命周期互不串联。
- 删除外置窗口顶部第二层“独立面板”标题栏；最小化 `—` 与真正的 X 放在聊天面板标题行同一水平线上。
