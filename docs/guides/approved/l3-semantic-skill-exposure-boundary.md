...
- **未来允许范围**：为 `CertifiedMotionLibrary.Entry` 增加 `LlmExposed` 声明、按本边界维护聊天提示与 `request_body_skill` 的枚举/执行双重拒绝、对应 EditMode 测试及直接文档。
- **禁止范围**：借暴露边界新增技能或认证条目、放开未授权技能的可见性、恢复原始参数或关键帧入口、启用主动或自主身体行为、任何运行时云端调用。
- **阻断条件**：把未认证候选标记为可见、改变“默认不可见”语义、或与产品方向基线 §4 及 [L3 认证指导](l3-embodied-skill-certification.md) 的授权规则冲突时，必须 `BlockedByDecision` 并停止实施。
