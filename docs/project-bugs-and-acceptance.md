...
> **2026-08-16 当前验证**：外置窗口退出线程、审批覆盖层及 `WindowOverlay` 排除外置窗口改动已通过 Quick、完整构建、EditMode 78/78 和隔离冒烟测试；P1 `destroyTJDevice` 是否在真实退出时彻底消失，仍按原验收要求保留观察项。
...
> **开发历史迭代**: 2026-08-16 外置面板 A1-A6/B1-B3 阶段集中产出 10+ 个渲染/交互 Bug（多数已修）；N44 测试模式禁云端新增一类"测试与生产行为偏差"。
...
| # | 问题 | 影响 | 状态 | 应对 |
|---|------|------|------|------|
| P1 | **destroyTJDevice 退出崩溃**（崩溃计数 105→108→120 持续增长） | 退出时引擎崩溃 → 反复重启 → 叠加烧钱嫌疑（见 P4） | 🟡 **已降级未确认**：`2cad357` 按 DisableExternalMode→Shutdown→释放 RT/NativeArray 顺序修复；退出时引擎崩溃是否彻底消失**未最终确认**（exit-time 崩溃不影响外部交互） | 多轮真实退出观察 Player.log；若复现按"退出崩溃"专项排查 |
...
| P4 | **¥5/天消耗源头未定位**：pet 侧 DeepSeek 调用量实测很小（1-2 次/会话） | 疑点：崩溃反复重启（P1 叠加）、Ollama 未就绪时本地优先回退云端、定时触发源（IdleChat/ServerPoll/ProactiveMessageScheduler） | 🔴 未解决 | **生产连续跑数天收集 `usage_log.jsonl` 分源数据**再定位，不凭猜测 |
| P5 | **外置窗口退出顺序回归风险**：OnDestroy 顺序一旦被改（DisableExternalMode→Shutdown→释放资源） | 窗口线程泄漏/崩溃 | 🟡 已修复需防回归 | 改 OnDestroy/窗口生命周期代码后必须跑冒烟测试 + 退出观察 |
| P6 | **多屏/分辨率降级恒不可用**：`isMultiMonitor` 恒 false、`GetResolutionScale` 恒 1.0 | 规划中能力，勿宣称可用 | 💡 遗留项 | 勿修（见 task-inventory 遗留项） |
