...
> **2026-08-29 当前核对**：沿用 2026-08-26~27 的 Quick、完整构建和隔离冒烟测试证据；本次仅完成文档与代码状态对齐，没有把历史测试结果冒充新鲜结果。P1 `destroyTJDevice` 是否在真实退出时彻底消失，仍按原验收要求保留观察项。此前记录的约 ¥5/天异常已确认来自其他项目复用同一个用户级 `DEEPSEEK_API_KEY`，不归因于桌宠项目。
