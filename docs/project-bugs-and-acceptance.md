...
> **2026-09-04 节日专项状态**：新春、元宵、端午、七夕、中秋已完成代码实现、隔离自动验证和四类 Unity 截图视觉预审，预评分分别为 91/92/91/91/91，当前未登记 P0/P1/P2。真实 GUI 双击展开和拖拽/收回仍是独立人工验收门槛，未取得稳定签字前，五个主题的 T3/T5、T6 及 G1～G5 均不得标记为最终完成；本条不改变上方构建高温 P0 和退出崩溃 P1 的既有结论。
...
### 1.1 测试实例单实例锁隔离（2026-09-04）

此前 `DesktopPet` 使用固定全局互斥名，导致已有生产播放器时，使用临时 `FU_XUAN_DATA` 的节日皮肤评测实例也会被错误拦截。现已改为按规范化 `DataPathConfig.DataRoot` 生成稳定哈希互斥名：同一数据根仍禁止重复实例，不同隔离数据根可并行启动。Quick、完整构建、EditMode、隔离冒烟、端午逐主题评测及双数据根并行启动均通过；后续修改启动互斥逻辑必须回归上述检查。
