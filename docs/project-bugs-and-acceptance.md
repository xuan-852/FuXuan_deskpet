...
> **2026-08-29 当前核对**：本轮目标点式拖拽物理输入代码已通过 Quick 构建和隔离运行冒烟；可见播放器中的挣扎观感、GPU/帧率和裁切仍需人工确认。P1 `destroyTJDevice` 是否在真实退出时彻底消失，仍按原验收要求保留观察项。此前记录的约 ¥5/天异常已确认来自其他项目复用同一个用户级 `DEEPSEEK_API_KEY`，不归因于桌宠项目。
