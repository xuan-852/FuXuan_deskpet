...
> **2026-08-29 当前核对**：本轮目标点式拖拽物理输入、响应降幅和测试截图闭环已通过 Quick/完整构建、隔离运行冒烟及左右拖动（含镜像后）截图回归；GPU/帧率和裁切仍需人工测量确认。P1 `destroyTJDevice` 是否在真实退出时彻底消失，仍按原验收要求保留观察项。此前记录的约 ¥5/天异常已确认来自其他项目复用同一个用户级 `DEEPSEEK_API_KEY`，不归因于桌宠项目。
