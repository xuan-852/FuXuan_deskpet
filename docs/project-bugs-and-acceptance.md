...
> **2026-08-26 当前核对**：外置窗口退出线程、审批覆盖层及 `WindowOverlay` 排除外置窗口改动此前已通过 Quick、完整构建和隔离冒烟测试；本轮又验证了统一退出清理和建窗早期关闭请求保护。P1 `destroyTJDevice` 是否在真实退出时彻底消失，仍按原验收要求保留观察项。此前记录的约 ¥5/天异常已确认来自其他项目复用同一个用户级 `DEEPSEEK_API_KEY`，不归因于桌宠项目。
