...
| **4** | 虚拟机验收（§八清单）+ 版权/密钥文档 + 发布流程（构建→打包→版本号） | 0.5~1 天 | 🔄 进行中（2026-08-15 腾讯云 2核4G Windows Server 实测：**安装产物 5/5、环境变量 5/5、NSSM 桥服务 RUNNING、桌宠启动/零NRE/透明窗口 全 PASS**；唯一未决项 `GET /health` 连接拒绝 = 桥进程在弱机上启动时被资源拖崩，非安装包缺陷（桥在本机 PM2/便携双路均验证正常）；**2核4G 跑桌宠验收内存不足导致死机**，报告截断于聊天项——建议后续用 4核8G 实例或本机 VirtualBox 复验 /health 与聊天项；本机另有完整验收记录 `installer\dist\acceptance-report-local.txt`（15 PASS/7 预期差异）） |
| 合计 | | **4~6 天** | |
