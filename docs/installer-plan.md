...
> **当前状态**: ✅ **阶段 0 已完成（2026-08-14）**——三个移植障碍代码改造全部落地并验证；✅ **阶段 1 便携原型已完成（2026-08-14）**——`installer\build-portable.ps1` 组装 `installer\portable\`，真机验证通过；✅ **阶段 2 安装器已完成（2026-08-15）**——`installer\fuxuan-installer.iss` + `build-installer.ps1` 产出 `installer\dist\FuXuanSetup-1.0.0.exe`（112.9MB，中文向导 + API 密钥收集页 + 数据目录选择 + 环境变量写入 + 卸载保留数据），本地静默安装/卸载验证通过；⚠️ 遗留：Python embeddable 内置待补测、桥服务注册（NSSM）与 OpenClaw/Ollama 组件自动化归入阶段 3、干净 VM 全清单验收归入阶段 4。
