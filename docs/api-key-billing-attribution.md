...
此前约 ¥5/天的异常消耗已由用户确认：费用来自其他项目复用同一个用户级 `DEEPSEEK_API_KEY`，不是桌宠项目自身产生。桌宠的 `usage_log.jsonl`、`key_id` 和 `key_hash` 仍用于记录桌宠进程实际调用的 Key 身份，但不能把同一 Key 的其他项目调用归入桌宠。

因此，该事项从“桌宠消耗源头未定位”调整为“跨项目共用 Key 的历史归因已确认”。后续如需核对平台账单，仍应给不同项目配置独立 Key，或使用运行身份和平台后台记录按项目交叉核对。
