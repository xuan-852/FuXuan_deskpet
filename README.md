...
| 🎉 **像素节日适配** | 已实现新春、元宵、端午、七夕、中秋 5 个中国传统节日主题，覆盖像素符玄配饰、聊天窗口/气泡和独立动态背景；动态时钟与透明窗口主动重绘已修复并完成复测；已完成自动验证与视觉预审，最终真实 GUI 双击/拖拽签字仍待完成；Live2D 保持不变 |
...
.\build.ps1 -RunTests    # 运行 Editor 测试套件（当前 160 用例，含 1 个环境忽略）
