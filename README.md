...
- **3D 模型骨架** — `HybridRenderer` + `Model3DRenderer` 混合渲染框架（0.3s 交叉淡入淡出代码已就绪，当前 3D 模型未接入，恒走 Live2D 分支）

### 🤖 AI 对话系统
- **DeepSeek Chat API** — 完整 Function Calling 支持（48+ 法术工具）
- **工具分类**：
  - 🔭 **观星术** — 打开网页、Bing 联网搜索
  - 📷 **摄形术** — 截图 + GLM-4V 视觉分析
  - 🎵 **调音术** — 调节音量、静音切换
  ...
  - 🧠 **忆境术** — 读取/写入长期记忆
  - 📚 **藏书阁** — 本地知识库语义检索（Ollama nomic-embed-text 嵌入 + RAG）
...
- **本地知识库（KnowledgeBaseManager）** — 对指定文件夹进行分块索引 + Ollama 嵌入语义检索，支持 .cs/.py/.md/.txt 等 25+ 文件类型
- **对话关联检索** — 每次对话后自动提取关键词搜索知识库，缓存结果注入 SystemPrompt
- **索引工具** — `knowledge_search` 语义检索 + `knowledge_index` 索引文件夹/文件
- **反思反射** — 累积重要性 ≥ 30 时触发 LLM 提炼高层次洞察
- **话题冷却** — 相同话题 120s 冷却，防止重复记录

### 🧬 人格演化系统（V3 新功能）
- **五维人格特质**（PersonalityManager）— 勤勉/亲和/活泼/自信/求知，各 0~1 渐变
- **三维关系模型** — 信任/亲密/熟悉度，随交互累积演化
- **交互驱动微调** — 每次对话后根据用户消息内容、活动类型、工具成功率自动调整
- **人格↔情绪联动** — 人格基线影响 EmotionState 四维基值（自信→效价高，活泼→激活度高）
- **长时间无交互回归** — 超过 300s 无人时人格缓慢回归中性
- **里程碑系统** — 每 20 次交互 / 亲密度突破阈值自动纪录人格成长印记
- **JSON 持久化** — pet_personality.json，跨会话保持人格连续演化
- **自省工具** — `inspect_personality` 术式让 AI 可查看自身人格与关系状态
...
│       │   │   ├── ToolCallInvoker.cs         # 46+ 法术工具执行（Newtonsoft.Json）
...
│       │   │   ├── KnowledgeBaseManager.cs    # 藏书阁 — 本地 RAG 知识库
│       │   │   ├── PetConfig.cs               # 天机簿配置
...
│       │   │       ├── PersonalityManager.cs   # 人格特质与关系演化系统
│       │   │       └── 参数知识层             # 8 文件 - 参数语义映射
...
│  │     │    ├── ToolCallInvoker (46+ 工具, Newtonsoft.Json)          │  │
...
JSON 持久化 (DataPathConfig.DataRoot = D:\DesktopPetData\):
  ...
  activity_log.json ──▶ ActivityTracker (活动记录)
  ActionRefs/       ──▶ ActionReferenceManager (动作截图)
  glm_collages/     ──▶ DualModelValidator (GLM-4V 拼图缓存)
  knowledge_base.json──▶ KnowledgeBaseManager (本地知识库)
```
