...
- **任意位置拖拽** — 拖拽 + 抛掷物理（速度缓冲平均，`throwScale=0.5`）
- **拖拽挣扎动画** — 双臂划水 + 双腿交替 + 扭动 + 慌张表情（20+ 头发/裙子参数直接驱动）
- **分区点击反馈** — 头部（摸头眯眼）/ 身体（戳胸惊讶）/ 腿部（害羞踢腿）
- **鼠标眼睛跟随** — 眼球平滑追踪鼠标位置（150px 触发距离）
- **屏幕边缘碰撞反弹** — 撞墙动画 + 反弹物理

### 🎭 动作系统
- **12 种空闲动作** — JSON 配置驱动（歪头/微笑/挑眉/星辉/伸懒腰/委屈/法阵/害羞/困惑/爱心等）
...
- **闭环自评** — `DualModelValidator` 调用 GLM-4V 视觉模型评估动作质量，自动优化

### 🧠 记忆与感知
- **「法眼」活动追踪** — 2s 轮询前台窗口，按分类（编程/游戏/学习/浏览等）累计时长
- **多窗口环境感知** — EnumWindows 扫描所有可见顶层窗口
- **浏览器标签深度感知** — 通过 UI Automation 反射读取浏览器标签页
- **分层长期记忆** — 核心事实始终保留 + Top-5 重要记忆 + 近期琐事
- **反思反射** — 重要性积分累计达阈值时触发 LLM 提炼洞察
- **话题冷却** — 相同话题 120s 冷却，防止重复记录

### 🌤️ 时间与天气
- **昼夜感知** — 自动检测系统时间，夜间/深夜特化表情
- **双天气源** — 和风天气（需 Key）优先，失败自动回退 wttr.in（无 Key）
- **AI 天气语录** — DeepSeek 生成符玄风格的天气台词（每次 6 条）

### 📋 便签与提醒
- **本地 JSON 持久化** — 增删改查，支持优先级
- **重复规则** — 一次性/每日/工作日/每周
...
- **悬浮球 (BallPanel)** — 桌面右下角粉色 ✦ 悬浮球，单击展开辐射菜单
  ...
  - 📊 报告面板：演武心经报告
  - 📋 便签面板：待办/已完成管理，CRUD
  - 可拖拽标题栏（420×580px），右键关闭
- **右侧面板 (RightPanel)** — Windows 11 Widgets 风格
  - `~` 键切换，鼠标划过右边缘自动展开（1s 自动隐藏）
  - 4 个工具按钮：聊 / 设 / 签 / 告
  - 底部输入栏集成，展开 220px / 收起 8px
- **底部输入栏** — Windows 搜索风格居中输入框
- **古风聊天气泡** — 紫色主题 + 星点闪烁动画
- **优先级气泡系统** — High（AI 回复）> Normal（提醒）> Low（闲话）

### 🖥️ 窗口系统
- **DWM 透明窗口** — WS_POPUP 全屏 + DwmExtendFrameIntoClientArea
- **点击穿透** — 每帧动态设置 WS_EX_TRANSPARENT，DragHandler 管理区域
- **多显示器支持** — 虚拟桌面全覆盖
- **睡眠唤醒恢复** — 帧间时间间隙检测 + 延迟重建 DWM
- **DWM 崩溃安全模式** — 连续 5 次重建失败跳过
- **系统托盘** — 左键隐藏/显示 + 右键菜单（开机自启/退出）
- **进程互斥** — Mutex 防止多实例

### ⚡ 性能优化
- **智能性能监控** — FPS 滚动 90 帧采样，三级自适应
- **CPU/GPU 升档拦截** — 占用 > 80% 阻止升档，> 92% 紧急降 Low
- **NVML 支持** — NVIDIA GPU 占用率监控
...
│       │   │       ├── ActionAgent/           # 动作代理核心（15 个组件）
│       │   │       ├── ActionPresets/         # 动作预设（6 个组件）
│       │   │       └── 参数知识层（6 个组件）
│       │   ├── Editor/
...
| GLM API Key（可选） | 环境变量 `GLM_API_KEY`（视觉分析 + 动作自评） |
...
│  │  │  │   └── 底部输入栏                                      │  │  │
│  │  │  │                                                        │  │  │
│  │  │  +── AI 系统 (ChatManager → ToolCallInvoker)            │  │  │
│  │  │  │   ├── DeepSeek Function Calling (37+ 工具)            │  │  │
│  │  │  │   ├── 多轮回环 (≤5轮) + 协程异步执行                  │  │  │
│  │  │  │   └── 句子逐句显示                                     │  │  │
│  │  │  │                                                        │  │  │
│  │  │  +── 动作系统 (Live2DRenderer + ActionAgent)             │  │  │
│  │  │  │   ├── Perlin 微动 / 眨眼呼吸 / 空闲动作调度            │  │  │
│  │  │  │   ├── 走路/拖拽/点击/撞墙动画                         │  │  │
│  │  │  │   ├── LLM 动作翻译 (MotionTranslator)                  │  │  │
│  │  │  │   └── 闭环自评 (GLM-4V → MotionMemoryManager)        │  │  │
│  │  │  │                                                        │  │  │
│  │  │  +── 感知系统 (ActivityTracker/PetMemory/Weather)        │  │  │
│  │  │  +── 服务系统 (Reminder/ServerPoll/Performance)          │  │  │
│  │  │  +── 系统集成 (WindowOverlay/SystemTray)                 │  │  │
│  │  └───────────────────────────────────────────────────────┘  │  │
│  └─────────────────────────────────────────────────────────────┘  │
└───────────────────────────────────────────────────────────────────┘
```
...
用户操作（点击/拖拽/键盘/~键）
    │
    ▼
DragHandler / BallPanel / RightPanel / BottomInputBar
    │
    ├──▶ DesktopPet (物理更新) ──▶ Live2DRenderer (参数)
    │
    ├──▶ BallPanel → PetConfig / MotionMemoryManager / ReminderManager
    │
    ├──▶ RightPanel → BallPanel 标签页 / ChatManager 输入
    │
    └──▶ ChatManager (AI 对话)
              │
              ├──▶ ToolCallInvoker (37+ 法术)
              └──▶ ChatBubble (逐句显示 + 优先级)
```
...
Live2DRenderer.LateUpdate(801)  → 覆盖被物理重置的参数
```
