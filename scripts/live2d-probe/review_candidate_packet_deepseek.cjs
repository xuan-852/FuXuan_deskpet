/* Controlled DeepSeek review for one frozen candidate-action packet.
 * Usage: node scripts/live2d-probe/review_candidate_packet_deepseek.cjs <isolated-root> <skill-id>
 * It verifies every frame hash before sending the six deterministic review phases.
 */
'use strict';
...
const fingerprint = crypto.createHash('sha256').update('candidate-packet-deepseek-v1' + packet.packetSha256 + prompt).digest('hex');
const reviewDir = path.join(root, 'cloud_review');
fs.mkdirSync(reviewDir, { recursive: true });
const output = path.join(reviewDir, `${skillId}-deepseek-${fingerprint}.json`);
...
(async () => {
  const response = await fetch('https://api.deepseek.com/chat/completions', {
    ...
    body: JSON.stringify({ model: 'deepseek-v4-flash', temperature: 0, max_tokens: 360, thinking: { type: 'disabled' },
  ...
  const result = { schema: 'live2d-candidate-deepseek-review/v1', skillId, packetSha256: packet.packetSha256, fingerprint,
    provider: 'deepseek', model: 'deepseek-v4-flash', status: response.ok ? 'ok' : 'http_error', httpStatus: response.status,
    frames: reviewFrames.map(frame => ({ order: frame.order, phase: frame.phase, sha256: frame.sha256 })), content: response.ok ? content : '', verdict,
    error: response.ok ? null : (payload.error?.message || 'request rejected'), usage: payload.usage || null };
  fs.writeFileSync(output, JSON.stringify(result, null, 2) + '\n', 'utf8');
  console.log(JSON.stringify({ output, status: result.status, usage: result.usage, verdict: result.verdict }, null, 2));
