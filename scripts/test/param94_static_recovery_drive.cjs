...
const root = path.join(os.tmpdir(), 'fuxuan_param94_static_recovery_20260916');
...
// Clear as soon as the newly written command has been consumed. This avoids a
// second candidate-start request from a later inbox polling interval.
...
(async () => {
    ...
    try {
        ...
        await sleep(3000); // wait for the initial automatic drop/landing
        await sendForOnePoll('@@sim:idle-actions:off');
        // Landing may have queued a normal edge-walk. Establish a measured
        // stationary baseline before attempting the candidate action.
        ...
        await waitFor('Released CandidateTest/param94-gesture#1: candidate-test-completed');
        console.log('candidate-released');
        await sendAndWait('@@sim:walk:right', '[TestInbox]');
        await sendAndWait('@@sim:status', 'velocity=(1,');
        console.log('post-action-walking-confirmed');
    }
    finally {
