 ...
 *   ② 结束现有 DesktopPet 实例（避免多窗叠影污染截图；现有实例为 .test_mode 测试实例）
 *   ③ 启动桌宠 → @@view:open → 逐主题 @@sim:holiday:<id> → static/motion 截图 → off → recovery 截图
...
const DATA_ROOT = process.env.FU_XUAN_EVAL_DATA || path.join(os.tmpdir(), 'fuxuan_festival_eval');
...
function killPet() {
  try { execSync('taskkill /IM DesktopPet.exe /F /T', { stdio: 'ignore', windowsHide: true }); } catch { /* 无实例则忽略 */ }
}
