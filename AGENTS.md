...
>
> **AI 读文档优先级**：`AGENTS.md` → `docs/README.md` → `development-standards.md` → `build-workflow.md`（**改 C# 后构建 / 构建卡死时先读**）→ `code-truth-architecture.md` → `docs/token-cost-testing.md`（涉及云端调用/测试/排查烧钱时**必须先读**）→ `docs/token-saving-architecture.md`（设计/修改成本控制时**必须先读**）→ `docs/project-bugs-and-acceptance.md`（改外置窗口/渲染/退出/测试代码前**必须先读**）→ 对应 `docs/modules/<模块>.md`
